package com.gmail.puhovashablinskaya.controller.security.filter;
...
public class AuthTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            if (jwt != null && jwtUtils.validateJwtToken(jwt) &&
                    sessionService.isActiveSession(jwt) &&
                    userService.isActiveUser(username)
            ) {
                ...
                detailsService.saveDateOfAuth(username);
            }
        } catch (Exception e) {
    ...
    private String parseJwt(HttpServletRequest request) {
        ...
        if (StringUtils.hasText(headerAuth) && headerAuth.startsWith(PREFIX_BEARER)) {
            return headerAuth.substring(PREFIX_BEARER.length());
        }
        return null;
    }
}
